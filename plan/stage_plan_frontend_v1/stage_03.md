...
## Todo List
- [ ] 时间轴缩放体系：水平缩放（鼠标滚轮/快捷键）、时间刻度自适应、定位到选区。
...
- [ ] 历史与撤销重做：对 timeline 操作、script 编辑、生成替换提供统一 undo/redo（至少覆盖关键路径）。
- [ ] 冲突与保护：生成回填时若用户已手动编辑，提示“替换/并存/取消”，避免无声覆盖。
- [ ] 大项目性能：timeline 虚拟化渲染、重计算隔离、避免全局 store 频繁更新导致卡顿。
- [ ] 错误边界与恢复：Editor 级 error boundary；子模块崩溃不拖垮整页，并提供“重载/上报信息”入口。
