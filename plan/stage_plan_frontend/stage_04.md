...
## Todo List
- [ ] 引入 QueryClient Provider 并定义全局默认策略（retry、staleTime、refetchOnWindowFocus）。
- [ ] 将 feed / user profile / project list / graph 等查询迁移到 query hooks。
- [ ] 将 like / fork 等 mutation 迁移到 mutation hooks，并实现乐观更新/失效刷新。
- [ ] 统一 loading/empty/error UI（复用 Stage 02 组件）。
- [ ] 明确“编辑器数据保存”的策略：手动保存 + 防抖自动保存 + 离开页面提醒。
